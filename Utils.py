...
from scapy.all import Raw, socket, StreamSocket, chexdump
from mqtt import  MQTT, MQTTConnect, MQTTConnack, MQTTSubscribe,  \
MQTTPublish, MQTTPuback, MQTTPubrec, MQTTPubrel, MQTTPubcomp, CONTROL_PACKET_TYPE, ByteField, ShortField
...
class Utils:
    ...
    def init_logger(self, name, log_file, level=DEBUG):
    ...
    def fuzz_by_param(self, params, type_flag, fuzz_data):
        ...
        if type_flag == "PUBACK":
            with open("templates/pub-sub-status.yaml", "r") as stream:
                fuzz_dict = safe_load(stream)
        ...
        if type_flag == "PUBLISH":
            self.stream_sock.sr1(Raw(MQTT()/MQTTConnect(clientId="client_"+str(randint(100, 999)), protoname="MQTT")), timeout=0, verbose=0)
    ...
    def send_pkt(self, pkt):
        ...
        try:
            ...
            self.stream_sock.sr1(Raw(pkt), timeout=0, verbose=0)
